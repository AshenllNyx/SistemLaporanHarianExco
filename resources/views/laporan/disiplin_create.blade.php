...
<div style="margin-bottom: 32px;">
    ...
    <p style="color: #64748b; font-size: 15px;">Mencatat kes-kes disiplin atau isu sahsiah pelajar.</p>
</div>
...
<div style="background:white;padding:32px;border-radius:24px;box-shadow:0 10px 40px rgba(0,0,0,0.05);border:1px solid #f1f5f9">
    <form method="POST" action="{{ route('laporan.disiplin.store') }}">
        ...
        <div style="margin-bottom:24px">
            <label style="font-weight:700;display:block;margin-bottom:12px;color:#1e293b;font-size:15px">👥 Pilih Pelajar Terlibat</label>
            ...
            <button type="button" id="add-student-btn" style="padding:10px 16px;border-radius:12px;background:#f0fdf4;color:#16a34a;border:1px solid #dcfce7;font-size:14px;font-weight:700;cursor:pointer;margin-top:8px">
                + Tambah Pelajar Lain
            </button>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 24px; margin-bottom: 24px;">
            <div>
                <label style="font-weight:700;display:block;margin-bottom:8px;color:#1e293b;font-size:15px">⚠️ Jenis Kesalahan</label>
                <input type="text" name="jenis_kesalahan" required 
                       style="width:100%;padding:12px;border-radius:12px;border:1px solid #e2e8f0;font-size:14px" 
                       placeholder="Cth: Merokok, Ponteng, Vandalisme" />
            </div>

            <div>
                <label style="font-weight:700;display:block;margin-bottom:8px;color:#1e293b;font-size:15px">🛠️ Tindakan Diambil</label>
                <input type="text" name="tindakan" 
                       style="width:100%;padding:12px;border-radius:12px;border:1px solid #e2e8f0;font-size:14px" 
                       placeholder="Cth: Nasihat, Lapor Warden, Denda" />
            </div>
        </div>
        ...
        <div style="margin-bottom:32px">
            ...
            <textarea name="catatan" rows="4" 
                      ...
                      placeholder="Masukkan butiran lanjut jika perlu..."></textarea>
        </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #f1f5f9; padding-top: 32px; flex-wrap: wrap; gap: 20px;">
            <a href="{{ $from === 'hub' ? route('laporan.edit', $laporan->id_laporan) : route('laporan.kerosakan.soalan', $laporan->id_laporan) }}" 
               class="btn-back"
            ...
            <button type="submit" 
                    ...
                    style="background: #2563eb; color: white; padding: 14px 32px; border-radius: 14px; border: none; font-weight: 700; cursor: pointer; box-shadow: 0 4px 12px rgba(37,99,235,0.2); font-size: 15px; flex: 1; min-width: 250px;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderOptions(selectEl, options, searchTerm) {
        ...
        if (filtered.length === 0) {
            box.innerHTML = '<div class="select-option empty">Tiada padanan. Cuba taip nama atau no. IC.</div>';
            return;
        }
    ...
    addBtn.addEventListener('click', function() {
        ...
        var removeBtn = clone.querySelector('.remove-student-btn');
        if (!removeBtn) {
            removeBtn = document.createElement('button');
            removeBtn.type = 'button';
            removeBtn.className = 'remove-student-btn';
            removeBtn.style.cssText = 'padding:8px 12px;border-radius:8px;background:#ef4444;color:white;border:none;font-size:14px';
    ...
    var firstWrapper = container.querySelector('.student-select-wrapper');
    if (!firstWrapper.querySelector('.remove-student-btn')) {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'remove-student-btn';
        btn.style.cssText = 'padding:8px 12px;border-radius:8px;background:#ef4444;color:white;border:none;font-size:14px';
        btn.textContent = 'Buang';
        btn.addEventListener('click', function() {
            if (container.querySelectorAll('.student-select-wrapper').length > 1) firstWrapper.remove();
        });
        firstWrapper.appendChild(btn);
    }
});
</script>
@endsection